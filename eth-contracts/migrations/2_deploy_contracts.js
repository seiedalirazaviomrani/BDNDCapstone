...
module.exports = function(deployer, network, accounts) {
  ...
  if (network == "rinkeby") {
    ...
    .then(() => {
        return deployer.deploy(SolnSquareVerifier, Verifier.address, "saro-BDND-CapstoneProject", "RSTSE", {from: accounts[0]})
        .then(async () => {
          // Store in client the contracts ABIs and addresses
          await fs.writeFileSync(__dirname + '/../../SolnSquareVerifier-Rinkeby.json',JSON.stringify(SolnSquareVerifier, null, '\t'), 'utf-8');
          return true;
        });
    });
    ...
    .then(() => {
        ...
        .then(async () => {
          ...
          await fs.writeFileSync(__dirname + '/../../config.json',JSON.stringify(config, null, '\t'), 'utf-8');
          // Store in server the contracts ABIs and addresses
          await fs.writeFileSync(__dirname + '/../../SolnSquareVerifier-Local.json',JSON.stringify(SolnSquareVerifier, null, '\t'), 'utf-8');
          // Store in client the contracts ABIs and addresses
          await fs.writeFileSync(__dirname + '/../../SolnSquareVerifier-Local.json',JSON.stringify(SolnSquareVerifier, null, '\t'), 'utf-8');
          return true;
        });
